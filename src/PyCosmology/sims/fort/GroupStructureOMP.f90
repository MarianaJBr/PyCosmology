  ...
  !=============================================================================
    subroutine structure_test(n,g_pos,n_radial_bins, n_angles,nside&
     ...
     !----------------- Variable Declaration ---------------------------------- 		
    	...
    	real(8), intent(out):: angles_c(n_angles)				!The diminishing angles (zeroth angle)
     ...
     !-------------------- 3 nested loops ------------------------------------
    	...
    	do mode=1,2
    		...
    		do k = 1,n_radial_bins
    			...
    			do h = 1, n_angles
    				write(*,*) "Currently Running ", omp_get_num_threads(), "threads"
    				if(mode==1)then
    	...
    	!$omp parallel private(i,j,angle)	
